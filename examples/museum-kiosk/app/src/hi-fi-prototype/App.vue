...
<template>
  <div ref="canvasRef" class="hf-canvas" @keydown="onKeydown" tabindex="-1">
    <!-- Floor plan (always visible) -->
    <div class="hf-map" :class="{ 'hf-map--dimmed': hasOverlay }">
    ...
    <div class="hf-hud">
      <div
        class="hf-hud__counter"
        :class="{ 'hf-hud__counter--bounce': counterBounce }"
        role="button"
        tabindex="0"
        @click="openDrawer('collection')"
        @keydown.enter="openDrawer('collection')"
      >
        <span>{{ totalCollected }} / {{ totalArtifacts }}</span>
      </div>
      <div class="hf-hud__nav">
        <button class="hf-hud__btn" title="Glossary" @click="openDrawer('glossary')">G</button>
        <button class="hf-hud__btn" title="About" @click="openDrawer('about')">?</button>
      </div>
    </div>

    <!-- Backdrop -->
    <Transition name="backdrop">
      <div v-if="hasOverlay" class="hf-backdrop" @click="activeRoomId ? closeRoom() : closeDrawer()"></div>
    </Transition>
